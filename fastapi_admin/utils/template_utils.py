    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if os.path.exists(project_template_dir):
                _copy_and_replace_template(project_template_dir, project_dir, {"PROJECT_NAME": project_name})
                
                # Create manage.py wrapper
                create_manage_py(project_dir, project_name)
            else:
                typer.echo("Error: Project template not found in the repository.")
                raise typer.Exit(code=1)
        except subprocess.CalledProcessError as e:
            typer.echo(f"Error: Failed to fetch template from {TEMPLATE_REPO}")
            typer.echo(f"Details: {e.stderr.decode() if e.stderr else 'Unknown error'}")
            raise typer.Exit(code=1)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if os.path.exists(app_template_dir):
                _copy_and_replace_template(app_template_dir, app_dir, {"APP_NAME": app_name})
            else:
